...
| 14. Process multiple decks safely | View-model tests assert top-level-only folder discovery in deterministic order, empty-folder handling, ordered sequential processing, isolated per-deck output directories, deterministic duplicate-name suffixes, aggregate completion status, continuation after one deck fails, graceful handling of OCR cleanup failure, and restoration of the interactive state after an unexpected conversion error. Batch-service tests independently assert folder ordering, direct single-file output, per-deck failure continuation, OCR option propagation, and deterministic machine-readable `batch-result.json`. Process-adapter tests assert worker progress/result recovery and conversion-failure reporting for a native-style worker exit code. Manually verify that the main window and CLI both advance to the next deck when one worker exits unexpectedly. |

The Windows workflow downloads checksum-pinned OCR models/test data, restores, builds, runs an actual Tesseract recognition smoke test, and publishes the WPF application, public CLI, and isolated verification worker into one self-contained directory with a shared .NET runtime. CI checks all three entry points, the local OCR native libraries, language models, app-local Visual C++ runtime files, and runtime configuration; it also confirms only one CoreCLR is packaged and both console executables start. The complete-package tests re-run a fixture twice and compare Markdown, JSON, report, and manifest byte-for-byte, verify every manifest asset's size and SHA-256, replace an intact prior package without stale assets, and refuse to overwrite unrelated files.
...
9. Select **Folder…** and choose a folder containing multiple `.pptx` files plus a nested subfolder; confirm only top-level presentations are queued in file-name order. Repeat by dropping the folder into the window.
10. In PowerShell, run `DeckContext\DeckContext.Cli.exe <folder> <output-root>` against the same folder. Confirm it produces the same top-level-only ordered batch layout, prints a final summary, writes machine-readable `batch-result.json`, and returns exit code `0` when every deck is `Succeeded` or `Partial`.
11. Confirm the five native charts retain chart type, series, categories, values, formulas/ranges, and chart-to-workbook linkage. Open exported `workbooks\*.xlsx` files and spot-check them against the presentation.
12. Leave **Recognize text inside images with offline OCR** enabled. Confirm screenshots with Chinese/English/Spanish text produce recognized text with `tesseract-local` provenance, nothing requests credentials or network access, repeated equal image/crop placements emit their transcription only once in Markdown, and an unreadable/unsupported image does not erase other slide content. Confirm Logo/icon/photo/map noise is omitted from Markdown with its bounded raw OCR and quality assessment retained in JSON. Disable OCR once and confirm images remain preserved with an explicit not-analyzed notice.
13. Confirm every unsupported/partial item appears in `extraction-report.json` with a useful code, severity, extractor, outcome, and source location; a local failure must not erase unaffected slides or objects.
14. Re-run the same deck into a fresh directory and compare the four primary text/JSON files. They should be byte-identical.
15. Give `deck.context.md` (and, when deeper traceability is needed, `deck.context.json`) to an LLM. Confirm it can identify slide structure, inspect table/chart data, read local-OCR transcriptions, distinguish OCR-derived text from native evidence, and use JSON when object-level geometry or provenance is needed.
